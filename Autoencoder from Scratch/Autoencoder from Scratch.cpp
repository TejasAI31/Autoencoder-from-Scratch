...
void AddNoise(vector<vector<double>>* input)
{
	...
	normal_distribution<double> dist(0,80);
...
void Display(vector<vector<double>>* input,vector<int> inputdims, vector<vector<double>>* predicted,vector<int> predicteddims ,vector<vector<double>>* actual)
{
	...
	while (!WindowShouldClose())
	{
		...
		{
			framecounter = 0;
			if(autotransition&&samplenum<input->size())
			samplenum++;
		}
		...
		if (CheckCollisionPointRec(mousepos, NextButton) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)&&samplenum<input->size())
			samplenum+=1;
		if (CheckCollisionPointRec(mousepos, PrevButton) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)&&samplenum>1)
			samplenum -= 1;
...
int main()
{
	...
	model.AddLayer(Layer(28*28, "LRelu"));

	vector<vector<double>> input;
	vector<vector<double>> actual;
	vector<vector<double>> predicted;

	CreateMnistDataset(&input, &actual, 500);
	AddNoise(&input);
